...
def fit(train, test, raw_train, raw_test, scaler, batch_size, nb_epoch, neurons):
    ...
    for i in range(nb_epoch):
        model.fit(X, y, epochs=1, batch_size=batch_size, shuffle=False, verbose = True)
    ...
    #predictions = model.predict(test, batch_size=1000)
    #predictions = np.expm1(predictions).clip(lower=0.)
